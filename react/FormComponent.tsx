...
import { messagesForm } from './shared'
...
const FormComponent: FC<FormComponentProps & InjectedIntlProps> = (props) => {
  ...
  const setDataForm = (
    ...
    setImgCategory(imgCategory ? imgCategory : "")
  }
  ...
  const returnHome = () => {
    const tab = responseForm.level === 'firstLevel' ? '1' : responseForm.level === 'secondLevel' ? '2' : '3'
    ...
    menuInput({
      variables: {
        editMenu: {
          ...
          imgCategory: mainMenuLevel.imgCategory
        },
      },
    })
  }
  ...
  const getNewPath = (slugPath: string, slugRootPath: string | undefined, slugRelativePath: string | undefined) => {
  ...
  const editItem = () => {
    ...
    if (responseForm.level === 'firstLevel') {
      ...
      const subMenuOtherLevels: MenuItem[] = menuLevelTwoUpdate.length > 0 ? menuLevelTwoUpdate : menuLevelTwo
                      ...
                      <>
                        {(responseForm.type === 'new' &&
                          ...
                          <>
                            ...
                            />
                            <div className="mb5 mt7">
                              <Input
                                placeholder=""
                                value={imgCategory}
                                label="Url da imagem da categoria"
                                id="imgCategory"
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => changeStyle({
                                  id: e.target.id,
                                  value: e.target.value,
                                })}
                              />
