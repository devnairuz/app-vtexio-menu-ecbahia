...
export const menus = async (
  ...
  try {
    ...
    logger.error({
      message: 'Error reading menuItems from VBase',
      status,
      errStr,
    })

    // Se for 404, inicializa o VBase com []
    if (status === 404 || errStr.includes('status code 404')) {
      logger.info({
        message: 'menuItems not found in VBase, initializing with empty array',
      })

      await vbase.saveJSON('menu', 'menuItems', [])
      menuItems = []
    } else {
      // Qualquer outro erro sobe pra quebrar mesmo
      throw err
    }
  }
